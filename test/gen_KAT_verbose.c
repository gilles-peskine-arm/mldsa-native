...
static void print_hex(const char *name, const uint8_t *data, size_t size)
{
  size_t i;
  printf("%s=", name);
  for (i = 0; i < size; i++)
...
int main(void)
{
  ...
  for (i = 0; i < MAXMLEN; i = (i == 0) ? i + 1 : i << 2)
  {
    ...
    print_hex("seed", coins, MLDSA_SEEDBYTES);
    print_hex("public", pk, CRYPTO_PUBLICKEYBYTES);
    print_hex("private", sk, CRYPTO_SECRETKEYBYTES);
    print_hex("random", coins + MLDSA_SEEDBYTES, MLDSA_RNDBYTES);
    print_hex("message", m, i);

    CHECK(crypto_sign_signature_internal(s, &slen, m, i, pre, sizeof(pre),
                                         coins + MLDSA_SEEDBYTES, sk, 0) == 0);

    print_hex("signature", s, slen);
